...
Verify that a samcoind node can load multiple wallet files
"""
...
class MultiWalletTest(SamcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another samcoind?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another samcoind?", self.nodes[0].loadwallet, wallet_names[0])
        else:
