# Bootstrappable Samcoin Core Builds

This directory contains the files necessary to perform bootstrappable Samcoin
Core builds.
...
To build Samcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Samcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/samcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/samcoin-core/guix.sigs).
