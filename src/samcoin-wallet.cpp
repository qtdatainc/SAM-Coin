...
static bool WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s samcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
        if (!args.IsArgSet("-version")) {
            strUsage += "\n"
                        "samcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default samcoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n"
                        "Usage:\n"
                        "  samcoin-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
int main(int argc, char* argv[])
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `samcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
