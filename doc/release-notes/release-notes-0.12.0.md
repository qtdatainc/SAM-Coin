Samcoin Core version 0.12.0 is now available from:

  <https://samcoin.org/bin/samcoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Samcoin-Qt (on Mac) or
samcoind/samcoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Samcoin Core or other software:
...
pre-0.12 versions of Samcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Samcoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Samcoin transactions now use validation using
[libsecp256k1](https://github.com/samcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/samcoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Samcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Samcoin Core was to raise the default minimum
relay fee.

Samcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Samcoin Core 0.12 also introduces new default policy limits on the length and
...
Samcoin Core 0.12 nodes. Samcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/samcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Samcoin Core 0.12 does not yet have support for
...
This allows running samcoind without having to do any manual configuration.
...
Samcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Samcoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Samcoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Samcoin Core 0.13, and it is at this time undecided whether the
...
Samcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Samcoin Core automatically creates a hidden service to listen on, without
manual configuration. Samcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Samcoin Core is listening, and
...
Samcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Samcoin
Core to use floating fees. By default, Samcoin Core will use floating
fees.
...
(default: `0.0002` SAM/kB).

At all times, Samcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SAM.
Furthermore, Samcoin Core will never create transactions paying less than
...
`samcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/samcoin/samcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/samcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/samcoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /samcoinrpc>
        ...
        # optional bypass samcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< samcoinrpc:<password>
    </Location>
...
Restarting samcoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Samcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new samcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include samcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix samcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of samcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override SAMCOIND and SAMCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [samcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add SAMCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libsamcoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "SAM" (Ross Nicoll)
