...
The Samcoin Core daemon can be configured to act as a trusted "border
router", implementing the samcoin wire protocol and relay, making
...
The ZeroMQ feature in Samcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building samcoind:
...
For instance:
    ...
    $ samcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/samcoind.tx.raw \
...
These options can also be provided in samcoin.conf.
...
From the perspective of samcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into samcoind directly. Furthermore, no information is
...
using. Samcoind appends an up-counting sequence number to each
